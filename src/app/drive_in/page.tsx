...
export default function DriveIN() {
	...
	return (
		<div className="h-[80dvh] flex flex-col gap-12 text-black">
			...
			{isRunning ? (
				<div className="text-center text-h2 text-active_green">
				...
				>
					{parkingMap?.map((item) => (
						<Marker
							...
							label={{
								text: item.available !== null ? item.available.toString() : 'fail',
								color: 'white',
								fontWeight: 'bold',
						...
						{!isRunning ? (
							<>
								<h1 className="text-center">
									Confirm with <span className="font-bold">{selectedPlace.name}</span>?
								</h1>
							...
							<h1 className="text-center">
								Location: <span className="font-bold">{selectedPlace.name}</span>
							</h1>
						)}
					</Box>
				</Modal>
